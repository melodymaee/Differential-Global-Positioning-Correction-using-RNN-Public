...
def mda(y_true, y_pred, t=12):
    d = tf.equal(tf.sign(y_true[t:] - y_true[:-t]),
                 tf.sign(y_pred[t:] - y_pred[:-t]))
    return tf.reduce_mean(tf.cast(d, tf.float32))
...
def build_SimpleRNN(input_shape, parameters):
    ...
    model.add(Dense(1))
    optimizer = parameters["optim"](lr=parameters["learning_rate"])
...
def build_LSTM(input_shape, parameters):
    ...
    model.add(Dense(parameters['FC_size'], activation='relu'))
    model.add(Dense(1))
    optimizer = parameters["optim"](learning_rate=parameters["learning_rate"])
    model.compile(loss='mse', optimizer=optimizer, metrics=[rmse, mda])
    return model
